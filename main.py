...
def prepare_relationship_sentence(rel_data):
    outgoing = " ".join(
        [
            f"OUT {r['type']} {r['neighbor_uri']}"
            for r in rel_data.get("outgoing_relationships", [])
        ]
    )
    incoming = " ".join(
        [
            f"IN {r['type']} {r['neighbor_uri']}"
...
property_similarity_matrix_labse = (
    torch.matmul(en_property_embeddings_labse, fr_property_embeddings_labse.T)
    .cpu()
    .numpy()
)
...
# Save the results
with open(
    "results/similar_entities_dbp15k_combined.json", "w", encoding="utf-8-sig"
) as f:
